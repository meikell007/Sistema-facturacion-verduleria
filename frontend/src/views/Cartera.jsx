...
export default function Cartera({ user }) {
  ...
  // ── Modal de abono ─────────────────────────────────────────────────────────
  const [abonoModal, setAbonoModal]       = useState(null); // factura seleccionada
  ...
  const [activeTab, setActiveTab]               = useState('cliente'); // 'cliente' | 'historial'
  ...
  const searchClients = async (q) => {
    try {
      ...
      // Solo clientes con deuda activa (credito_utilizado > 0) o todos activos
      setClientResults(Array.isArray(data) ? data.filter(c => c.activo) : []);
  ...
  const handleRegistrarAbono = async () => {
    ...
    try {
      ...
      setAbonoModal(null);
      // Recargar facturas pendientes y historial
      await loadPendingInvoices(selectedClient.id_cliente);
      await loadPaymentHistory();
      // Actualizar saldo visible del cliente seleccionado
      setSelectedClient(prev => ({
        ...prev,
        credito_utilizado: Math.max(0, (prev.credito_utilizado || 0) - monto)
      }));
    } catch (err) {
      const msg =
  ...
  const creditoDisp     = selectedClient
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.9rem' }}>
            Consulta deudas y registra abonos de clientes
          </p>
        </div>
      </div>

      {/* ── Modal de confirmación de abono exitoso ──────────────────────────── */}
      {abonoExito && (
        <div className="modal-overlay" onClick={() => setAbonoExito(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '420px' }}>
            <div className="modal-header">
              ...
              <button className="btn btn-outline" style={{ padding: '6px 10px' }} onClick={() => setAbonoExito(null)}>
                <X size={16} />
              </button>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setAbonoModal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '480px' }}>
            <div className="modal-header">
              ...
              <button className="btn btn-outline" style={{ padding: '6px 10px' }} onClick={() => setAbonoModal(null)}>
                <X size={16} />
              </button>
            </div>
            <div className="modal-body">
              ...
              <div style={{
                ...
                borderRadius: 'var(--radius-sm)', padding: '14px', marginBottom: '20px'
              }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <div>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', margin: '0 0 2px' }}>Factura</p>
                    <p style={{ fontWeight: 800, margin: 0 }}>{abonoModal.numero_factura}</p>
                  </div>
                  <div>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', margin: '0 0 2px' }}>Total original</p>
                    <p style={{ fontWeight: 700, margin: 0 }}>{formatCOP(abonoModal.total)}</p>
                  </div>
                  <div>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', margin: '0 0 2px' }}>Saldo pendiente</p>
                    <p style={{ fontWeight: 800, color: 'var(--warning)', fontSize: '1.05rem', margin: 0 }}>
                      {formatCOP(abonoModal.saldo_pendiente)}
                    </p>
                  </div>
                  <div>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', margin: '0 0 2px' }}>Días vencida</p>
                    <p style={{ fontWeight: 700, color: diasDesde(abonoModal.fecha_emision) > 30 ? 'var(--danger)' : 'var(--text-main)', margin: 0 }}>
                      {diasDesde(abonoModal.fecha_emision)} días
                    </p>
                  </div>
                </div>
              </div>
            ...
            {selectedClient ? (
              ...
              <div>
                <div style={{
                  ...
                  background: 'var(--primary-light)', borderRadius: 'var(--radius-sm)', padding: '14px 18px',
                  marginBottom: '16px'
                }}>
                  <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px' }}>
                  {[
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)', fontSize: '0.78rem', marginBottom: '6px' }}>
                        {m.icon} {m.label}
                      </div>
                      <p style={{ fontWeight: 800, color: m.color, margin: 0, fontSize: '1rem', fontFamily: 'var(--font-heading)' }}>
                        {m.value}
                      </p>
                    </div>
                  ))}
                ...
                {showDropdown && clientResults.length > 0 && (
                  <div style={{
                    ...
                    {clientResults.map(c => (
                      <div
                        key={c.id_cliente}
                        style={{ padding: '12px 16px', cursor: 'pointer', transition: 'background 0.15s', borderBottom: '1px solid var(--border)' }}
              ...
              {loadingInvoices ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {pendingInvoices.map(inv => {
                    ...
                    return (
                      <div key={inv.id_factura} style={{
                        display: 'grid', gridTemplateColumns: '1fr auto',
                        alignItems: 'center', gap: '16px',
                        padding: '16px', borderRadius: 'var(--radius-sm)',
                        border: `1px solid ${enMora ? 'var(--danger)' : 'var(--border)'}`,
                        background: enMora ? 'var(--danger-light)' : 'var(--bg-app)',
                      }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                            <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, color: 'var(--primary)' }}>
                              {inv.numero_factura}
                            </span>
                            <span className={`badge ${estadoBadge(inv.estado)}`} style={{ fontSize: '0.72rem' }}>
      ...
      {/* ── TAB: Historial global de abonos ─────────────────────────────────── */}
      {activeTab === 'historial' && (
        <div className="card" style={{ padding: '20px' }}>
                ...
                <tbody>
                  {paymentHistory.map(p => (
                    <tr key={p.id_pago}>
                      ...
                      <td style={{ color: 'var(--text-muted)', fontSize: '0.85rem', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {p.observacion || '—'}
                      </td>
                    </tr>
                  ))}
